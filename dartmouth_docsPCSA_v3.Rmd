...
dart_nyc$allpcp <- dart_nyc$pcpphys + dart_nyc$pa_pcp + dart_nyc$famprac + dart_nyc$internist
dart_nyc$allpcpnp <- dart_nyc$pcphys + dart_nyc$np_fte + dart_nyc$pa_pcp + dart_nyc$famprac + dart_nyc$internist
...
dart_nyc <- 
  mutate(dart_nyc, physician_rat = if_else(physicians !=0, (totpop/ physicians), NA_real_),
         ...
         allpcpnp_rat = if_else(allpcpnp !=0, (totpop/allpcpnp), NA_real_),
...
new10 <-
  dart_nyc %>%
  transmute(pcsa, year, gent_status, pcphys, specialist, obgyn, img_pcp, img_spec, img_ob, pa_pcp, pa_spec, pa_ob, pa_oth, fem15_44, totpop, pcpvt, edvt, physicians, pa, obstets, allpcp, allpcpnp, specs, allimg, physician_rat, pa_rat, allpcp_rat, allpcpnp_rat, specs_rat, obstets_rat, img_rat, pcpvt_rt, edvt_rt, pcpdenom, cnm_fte, np_fte, cnm_rat, np_rat, medicare_acscd, hospdenom, acscd_rt)
...
new10 <- 
  mutate(new10, 
         ...
         pa_p1000 = if_else(totpop !=0, (pa/totpop)*1000, NA_real_),
         allpcp_p1000 = if_else(totpop !=0, (allpcp / totpop)*1000, NA_real_),
         allpcpnp_p1000 = if_else(totpop !=0, (allpcpnp / totpop)*1000, NA_real_),
         specs_p1000 = if_else(totpop !=0, (specs/totpop)*1000, NA_real_),
         obstets_p1000 = if_else(totpop !=0, (obstets/totpop)*1000, NA_real_),
         img_p1000 = if_else(totpop !=0, (allimg/totpop)*1000, NA_real_),
