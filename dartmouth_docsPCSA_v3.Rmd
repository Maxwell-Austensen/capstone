...
```
```{r}
## if flip to rates - better for test?
doctable_ratesflip <-
  dart_all_years %>%
  group_by(gent_status, year) %>%
  summarise(physicians = sum(physicians)/sum(totpop) * 1000,
            ...
            obstets = sum(obstets) / sum(fem15_44) *1000, ## is this the right var??
...
sumtable_num <-
  ...
  summarise(physicians = sum(physicians),
            ...
            allpcp = sum(allpcp),
            specs = sum(specs),
            obstets = sum(obstets), ## is this the right var??
            img = sum(allimg)) %>%
  mutate_if(is.numeric, funs(round(., digits = 0)))
kable(sumtable_num)
```
...
poisson.exact(c(142, 224), c(393562, 318642),alternative = "t", tsmethod = c("blaker"), conf.level = .99) #obstets sig
...
poisson.exact(c(218, 467), c(561177, 462180),alternative = "t", tsmethod = c("blaker")) #obstets sig
...
poisson.exact(c(1021, 1249), c(973005, 748401),alternative = "t", tsmethod = c("blaker"), conf.level = .99) #obstets sig
...
poisson.exact(c(224, 467), c(318642, 462180),alternative = "t", tsmethod = c("blaker"), conf.level = .99) #obstets sig
...
poisson.exact(c(1249, 467), c(748401, 462180),alternative = "t", tsmethod = c("blaker"), conf.level = .99) #obstets sig
...
poisson.exact(c(224, 1249), c(318642, 748401),alternative = "t", tsmethod = c("blaker"), conf.level = .99) #obstets sig
