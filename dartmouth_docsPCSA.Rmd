...
gentxwalk$gent_status <- as.factor(gentxwalk$gent_status)
...
kable(vsttb %>% gather("var", "value", -gent_status) %>% spread(gent_status, value) %>% mutate(var = ordered(var, levels = order_vec)) %>% arrange(var))
